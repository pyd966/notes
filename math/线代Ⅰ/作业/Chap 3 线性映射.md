...
> 7. 设 $c_1,c_2,\dots,c_n$ 是 $n$ 个互异的实常数，证明：$R[x]_n$ 到 $R^n$ 的一个映射 $\sigma$：
>
> $$
> \sigma(p(x))=(p(c_1),p(c_2),\dots,p(c_n))
> $$
>
> 是 $R[x]_n$ 到 $R^n$ 的一个同构映射。
>
> 这里只要求对 $n=3$ 进行证明。

先证明 $\sigma$ 是线性映射。

设 $p(x)=a_0+a_1x+a_2x^2+a_3x^3,q(x)=b_0+b_1x+b_2x^2+b_3x^3$。

$\sigma(\lambda p(x)+\mu q(x))=(\lambda p(c_1)+\mu q(c_1),\dots,\lambda p(c_n)+\mu q(c_n))$

再证明 $\sigma$ 是双射。

单射：

反证法，假设 $\sigma(p(x))=(s_1,\dots s_n)$ 有两个解 $p_1(x)\neq p_2(x)$。

构造函数 $f(x)=p_1(x)-p_2(x)$，显然它的次数不超过 $2$。

由题可知，$f(c_1)=f(c_2)=f(c_3)=0$，也就是说 $f(x)$ 有三个不同零点，这是不可能的。

所以 $\sigma$ 是单射。

满射：

不难发现，$\text{Im}\sigma=L((1,1,1),(c_1,c_2,c_3),(c_1^2,c_2^2,c_3^2))$。

下面我们证明，这三个向量线性无关，进而构成 $R^3$ 的一组基。

写成矩阵的形式进行行初等变换化简成行简化阶梯矩阵。
...
\begin{pmatrix}
1 & c_1 & c_1^2\\
0 & c_2-c_1 & c_2^2-c_1^2\\
0 & 0 & (c_3-c_1)(c_3-c_2)\\
\end{pmatrix}
$$

由于 $c_1,c_2,c_3$ 互不相同，所以 $c_2-c_1\neq0,(c_3-c_1)(c_3-c_2)\neq0$，所以这三个向量线性无关。

所以 $\sigma$ 是一个满射。

综上，$\sigma$ 是一个线性双射，所以是 $R[x]_n$ 到 $R^n$ 的同构映射。