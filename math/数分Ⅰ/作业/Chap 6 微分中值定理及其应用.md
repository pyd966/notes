...
与 $y$ 轴交于 $(0,0)$，令 $y=0$ 得到与 $x$ 轴交于 $(0,0)$。

函数只在 $x=-1$ 处不连续。

$y'=\dfrac{x^2(x+3)}{2(1+x)^3}$，函数只在 $x=-1$ 处不可导。

分析函数的单调性，函数在 $(-\infty,-3],(-1,+\infty)$ 递增，在 $[-3,-1)$ 递减，所以函数有极大值点 $x=-3$。

$y''=\dfrac{3x}{(1+x)^4}$，所以函数在 $(-\infty,-1),(-1,0]$ 是凹函数，在 $[0,+\infty)$ 是凸函数，有拐点 $(0,0)$。
...
则 $k=\lim\limits_{x\to\infty}\dfrac yx=\lim\limits_{x\to\infty}\dfrac{x^2}{2(1+x)^2}=\dfrac12$

$b=\lim\limits_{x\to\infty}(y-kx)=\lim\limits_{x\to\infty}-\dfrac{2x^2+x}{2(1+x)^2}=-1$

所以函数的渐近线是 $x=-1,y=\dfrac12x-1$。

![alt text](chap6fig1.png)